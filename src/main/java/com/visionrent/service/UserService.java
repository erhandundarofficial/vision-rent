...
public class UserService {
    ...
    @Transactional // Veritabanı üzerinde gerçekleştirilen bir grup SQL işleminin tek bir bütün olarak ele alınmasını sağlar, bir veya daha fazla SQL işlemini tek bir işlem gibi ele alır
    ...
    public Set<Role> convertRoles(Set<String> pRoles) {
        ...
        if(pRoles==null) {
            ...
            pRoles.forEach(roleStr->{if(roleStr.equals(RoleType.ROLE_ADMIN.getName())) {/* Administrator */ Role adminRole = roleService.findByType(RoleType.ROLE_ADMIN); roles.add(adminRole);} else {Role userRole = roleService.findByType(RoleType.ROLE_CUSTOMER); roles.add(userRole);}});
        }
