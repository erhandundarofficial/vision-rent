...
public class CarService {
    ...
    public Car getCar(Long id) {
        Car car = carRepository.findCarById(id).orElseThrow(()-> new ResourceNotFoundException(String.format(ErrorMessage.RESOURCE_NOT_FOUND_MESSAGE, id)));
        return car;
    }
    ...
    public void updateCar(Long id, String imageId, CarDTO carDTO) {
       ...
       if(car.getBuiltIn()) { // builtIn kontrolü
       ...
       // burada amaç, verilen image daha önce başka araç için kullanılmış mı?
        ...
        for(Car c : carList) {
            //bana gelen car Id si ile yukarıdaki List türündeki car Id leri eşit olmaları lazım, eğer eşit değilse girilen image başka bir araç için yüklenmiş
